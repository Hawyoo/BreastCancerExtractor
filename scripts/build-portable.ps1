...
@("database\patients", "workspace", "models\llm", "models\ollama", "local_knowledge", "logs", "runtime\ollama") |
...
Write-Step 4 "Copy the optional Ollama standalone runtime"
if (-not $OllamaSource) {
    $OllamaCommand = Get-Command ollama.exe -ErrorAction SilentlyContinue
    if ($OllamaCommand) { $OllamaSource = Split-Path -Parent $OllamaCommand.Source }
}
if ($OllamaSource -and (Test-Path -LiteralPath (Join-Path $OllamaSource "ollama.exe"))) {
    Copy-Item -LiteralPath (Join-Path $OllamaSource "ollama.exe") `
        -Destination (Join-Path $PortableRoot "runtime\ollama\ollama.exe") -Force
    if (Test-Path -LiteralPath (Join-Path $OllamaSource "lib")) {
        Copy-Item -LiteralPath (Join-Path $OllamaSource "lib") `
            -Destination (Join-Path $PortableRoot "runtime\ollama") -Recurse -Force
    }
    Write-Host "Bundled Ollama runtime from: $OllamaSource" -ForegroundColor Green
}
else {
    Write-Warning "Ollama standalone was not found. Add it to runtime\ollama before offline distribution."
}
...
Copy-Item -LiteralPath "docs\WINDOWS_PORTABLE.md" `
    -Destination (Join-Path $PortableRoot "WINDOWS_PORTABLE.md") -Force
Write-Host ""
