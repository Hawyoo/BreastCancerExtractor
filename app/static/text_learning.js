...
(() => {
  ...
  async function collectTextLearningJson() {
    ...
    for (const patient of patients) {
      try {
        ...
        // A patient can disappear while the list is being scanned; skip only it.
      }
    }
    ...
    for (const detail of details) {
      ...
      for (const audit of detail.audit_log || []) {
        ...
        increment(item.corrections, key, {
          from,
          to,
          ...(reason ? {reason} : {}),
  ...
  function installLearningExportButton() {
    ...
    if (!actions || !improve || document.querySelector("#export-text-learning")) return;
  ...
  function rectFromOcrBox(box) {
    ...
    if (box.length >= 4 && box.every((value, index) => index >= 4 || Number.isFinite(Number(value)))) {
  ...
  function locateObservationEvidence(documentId, observationId) {
    ...
    if (!evidence) return [];
    const lines = ocrLinesForDocument(documentId);
    const candidates = lines.map((line, index) => {
      ...
      const score = relevance * 0.9 + ocrConfidence * 0.1;
      return {
        line_id: index + 1,
        text: text(line.text),
  ...
  function drawSmartEvidence() {
    ...
    for (const item of boxes) {
      ...
      const label = `文本定位 ${confidence}% · L${item.line_id}`;
  ...
  if (typeof openSavedDocumentPreview === "function" && !openSavedDocumentPreview.__bceTextLocationWrapped) {
    ...
    const wrappedOpen = async (documentId, observationId = null) => {
      ...
      if (observationId) {
        ...
        if (state.smartEvidenceBoxes.length) {
  ...
  function replaceUiTerminology(root = document.body) {
    ...
    const elements = root instanceof Element ? [root, ...root.querySelectorAll("[title],[aria-label],[placeholder]")] : [];
    for (const element of elements) {
      for (const attribute of ["title", "aria-label", "placeholder"]) {
        if (!element.hasAttribute(attribute)) continue;
        element.setAttribute(attribute, replaceValue(element.getAttribute(attribute) || ""));
      }
    }
  }

  function improveLearningDialogCopy() {
    const dialog = document.querySelector("#learning-summary-dialog");
    if (!dialog) return;
    const help = dialog.querySelector(".queue-help");
    if (help) help.textContent = "人工修改与人工补填会自动进入本地文本学习，并用于后续AI提取的纠错；这里仅展示汇总。可在患者列表上方导出JSON供AI或迁移使用。";
  }
